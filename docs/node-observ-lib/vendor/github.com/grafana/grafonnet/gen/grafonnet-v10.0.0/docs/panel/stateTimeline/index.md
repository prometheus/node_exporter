...
```jsonnet
fieldConfig.defaults.custom.withFillOpacity(value=70)
```
...
```jsonnet
fieldConfig.defaults.custom.withLineWidth(value=0)
```
...
```jsonnet
options.withMergeValues(value=true)
```
...
```jsonnet
options.withRowHeight(value=0.9)
```
...
```jsonnet
options.withTimezoneMixin(value)
```
