...
* [`obj options`](#obj-options)
  * [`fn withLimit(value=10)`](#fn-optionswithlimit)
...
```jsonnet
options.withNavigateAfter(value="10m")
```
...
```jsonnet
options.withNavigateBefore(value="10m")
```
...
```jsonnet
options.withNavigateToPanel(value=true)
```
...
```jsonnet
options.withOnlyFromThisDashboard(value=true)
```
...
```jsonnet
options.withOnlyInTimeRange(value=true)
```
...
```jsonnet
options.withShowTags(value=true)
```
...
```jsonnet
options.withShowUser(value=true)
```
