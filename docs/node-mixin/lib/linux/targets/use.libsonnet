...
{
  new(this): {
    local variables = this.grafana.variables.use,
    ...
    cpuUtilization:
      prometheusQuery.new(
        ...
        |||
          instance:node_cpu_utilisation:rate5m{%(queriesSelector)s} * 100 != 0
        ||| % variables,
      )
    ...
    cpuSaturation:
      prometheusQuery.new(
        prometheusDatasource,
        |||
          instance:node_load1_per_cpu:ratio{%(queriesSelector)s} * 100 != 0
        ||| % variables,
      )
    ...
    memoryUtilization:
      prometheusQuery.new(
        prometheusDatasource,
        |||
          instance:node_memory_utilisation:ratio{%(queriesSelector)s} * 100 != 0
        |||
        % variables
      )
      + prometheusQuery.withLegendFormat('{{%s}}: Utilization' % xtd.array.slice(this.config.instanceLabels, -1)),
    ...
    diskUtilization:
      prometheusQuery.new(
        prometheusDatasource,
        'instance_device:node_disk_io_time_seconds:rate5m{%(queriesSelector)s} * 100 != 0' % variables,
      )
      + prometheusQuery.withLegendFormat('{{%s}}: {{device}}' % xtd.array.slice(this.config.instanceLabels, -1)),
    diskSaturation:
      prometheusQuery.new(
        prometheusDatasource,
        'instance_device:node_disk_io_time_weighted_seconds:rate5m{%(queriesSelector)s} * 100 != 0' % variables,
      )
      + prometheusQuery.withLegendFormat('{{%s}}: {{device}}' % xtd.array.slice(this.config.instanceLabels, -1)),
    ...
    filesystemUtilization:
      prometheusQuery.new(
        ...
        |||
          sort_desc(1 -
            (
            max without (mountpoint, fstype) (node_filesystem_avail_bytes{%(queriesSelector)s, %(fsMountpointSelector)s, %(fsSelector)s})
            /
            max without (mountpoint, fstype) (node_filesystem_size_bytes{%(queriesSelector)s, %(fsMountpointSelector)s, %(fsSelector)s})
            ) != 0
          ) * 100
