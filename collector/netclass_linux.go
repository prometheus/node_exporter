...
func (c *netClassCollector) netClassSysfsUpdate(ch chan<- prometheus.Metric) error {
	...
	for _, ifaceInfo := range netClass {
		...
		infoDesc := prometheus.NewDesc(
			...
			[]string{"device", "address", "broadcast", "operstate", "adminstate", "ifalias"},
			nil,
		)
		infoValue := 1.0

		ch <- prometheus.MustNewConstMetric(infoDesc, prometheus.GaugeValue, infoValue, ifaceInfo.Name, ifaceInfo.Address, ifaceInfo.Broadcast, ifaceInfo.OperState, getAdminState(ifaceInfo.Flags), ifaceInfo.IfAlias)
