...
func Test_parseTCPStatsPerPort(t *testing.T) {
	...
	emptyDestPortStats, err := parseTCPStatsPerDestPort(msg, []string{"80"})
	if err != nil {
		t.Fatal(err)
	}

	if want, got := 0, int(emptyDestPortStats[sp1]); want != got {
		t.Errorf("expected 0 connections for unmatched port 80, got %d", got)
	}
}
