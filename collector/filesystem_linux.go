...
func (c *filesystemCollector) processStat(labels filesystemLabels) filesystemStats {
	...
	if float64(buf.Blocks)*float64(buf.Bsize) > float64(float64Mantissa) {
		size, _ := new(big.Float).SetUint64(buf.Blocks).Mul(new(big.Float).SetUint64(buf.Blocks), new(big.Float).SetInt64(buf.Bsize)).Float64()
		free, _ := new(big.Float).SetUint64(buf.Bfree).Mul(new(big.Float).SetUint64(buf.Bfree), new(big.Float).SetInt64(buf.Bsize)).Float64()
		avail, _ := new(big.Float).SetUint64(buf.Bavail).Mul(new(big.Float).SetUint64(buf.Bavail), new(big.Float).SetInt64(buf.Bsize)).Float64()
		return filesystemStats{
			labels:    labels,
			size:      size,
			free:      free,
