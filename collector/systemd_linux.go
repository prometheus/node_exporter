...
func (c *systemdCollector) Update(ch chan<- prometheus.Metric) error {
	...
	level.Debug(c.logger).Log("msg", "systemd getAllUnits took", "durations", time.Since(begin).Seconds())

	begin = time.Now()
	summary := summarizeUnits(allUnits)
	c.collectSummaryMetrics(ch, summary)
	level.Debug(c.logger).Log("msg", "systemd collectSummaryMetrics took", "durations", time.Since(begin).Seconds())

	begin = time.Now()
	units := filterUnits(allUnits, c.unitWhitelistPattern, c.unitBlacklistPattern, c.logger)
	level.Debug(c.logger).Log("msg", "systemd filterUnits took", "durations", time.Since(begin).Seconds())
	...
	go func() {
		...
		level.Debug(c.logger).Log("msg", "systemd collectUnitStatusMetrics took", "durations", time.Since(begin).Seconds())
	}()
	...
	if *enableStartTimeMetrics {
		...
		go func() {
			...
			level.Debug(c.logger).Log("msg", "systemd collectUnitStartTimeMetrics took", "durations", time.Since(begin).Seconds())
		}()
	}
	...
	if *enableTaskMetrics {
		...
		go func() {
			...
			level.Debug(c.logger).Log("msg", "systemd collectUnitTasksMetrics took", "durations", time.Since(begin).Seconds())
		}()
	}
	...
	if c.systemdVersion >= minSystemdVersionSystemState {
		...
		go func() {
			...
			level.Debug(c.logger).Log("msg", "systemd collectTimers took", "durations", time.Since(begin).Seconds())
		}()
	}
	...
	go func() {
		...
		level.Debug(c.logger).Log("msg", "systemd collectSockets took", "durations", time.Since(begin).Seconds())
	}()

	if c.systemdVersion >= minSystemdVersionSystemState {
		begin = time.Now()
		err = c.collectSystemState(conn, ch)
		level.Debug(c.logger).Log("msg", "systemd collectSystemState took", "durations", time.Since(begin).Seconds())
	}
...
func (c *systemdCollector) collectUnitStatusMetrics(conn *dbus.Conn, ch chan<- prometheus.Metric, units []unit) {
	for _, unit := range units {
		...
		if strings.HasSuffix(unit.Name, ".service") {
			...
			if err != nil {
				level.Debug(c.logger).Log("msg", "couldn't get unit type", "unit", unit.Name, "error", err)
			} else {
		...
		if *enableRestartsMetrics && strings.HasSuffix(unit.Name, ".service") {
			...
			if err != nil {
				level.Debug(c.logger).Log("msg", "couldn't get unit NRestarts", "unit", unit.Name, "error", err)
			} else {
				ch <- prometheus.MustNewConstMetric(
...
func (c *systemdCollector) collectSockets(conn *dbus.Conn, ch chan<- prometheus.Metric, units []unit) {
	for _, unit := range units {
		...
		if err != nil {
			level.Debug(c.logger).Log("msg", "couldn't get unit NAccepted", "unit", unit.Name, "error", err)
			continue
		}
		ch <- prometheus.MustNewConstMetric(
		...
		if err != nil {
			level.Debug(c.logger).Log("msg", "couldn't get unit NConnections", "unit", unit.Name, "error", err)
			continue
		}
		ch <- prometheus.MustNewConstMetric(
...
func (c *systemdCollector) collectUnitStartTimeMetrics(conn *dbus.Conn, ch chan<- prometheus.Metric, units []unit) {
	...
	for _, unit := range units {
		if unit.ActiveState != "active" {
			...
			if err != nil {
				level.Debug(c.logger).Log("msg", "couldn't get unit StartTimeUsec", "unit", unit.Name, "error", err)
				continue
			}
...
func (c *systemdCollector) collectUnitTasksMetrics(conn *dbus.Conn, ch chan<- prometheus.Metric, units []unit) {
	...
	for _, unit := range units {
		if strings.HasSuffix(unit.Name, ".service") {
			...
			if err != nil {
				level.Debug(c.logger).Log("msg", "couldn't get unit TasksCurrent", "unit", unit.Name, "error", err)
			} else {
			...
			if err != nil {
				level.Debug(c.logger).Log("msg", "couldn't get unit TasksMax", "unit", unit.Name, "error", err)
			} else {
...
func (c *systemdCollector) collectTimers(conn *dbus.Conn, ch chan<- prometheus.Metric, units []unit) {
	for _, unit := range units {
		...
		if err != nil {
			level.Debug(c.logger).Log("msg", "couldn't get unit LastTriggerUSec", "unit", unit.Name, "error", err)
			continue
		}
...
func getSystemdVersion(logger log.Logger) int {
	conn, err := newSystemdDbusConn()
	if err != nil {
		level.Warn(logger).Log("msg", "Unable to get systemd dbus connection, defaulting systemd version to 0", "error", err)
		return 0
	}
	defer conn.Close()
